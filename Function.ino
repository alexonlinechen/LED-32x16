...
void show05bitnumber(int number, int xlength, int ylength, int x, int y, uint16_t colorxy)
{
  ...
  if(number<10){
    ...
    showbitmap(bitdata05[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x, y, colorxy);
    showbitmap(bitdata05[(int)((String(numStr.charAt(1)).toInt() + 1) - 1)],xlength,ylength, x+4, y, colorxy);
    showbitmap(bitdata05[(int)((String(numStr.charAt(2)).toInt() + 1) - 1)],xlength,ylength, x+8, y, colorxy);
  }else if(number<10000){
    showbitmap(bitdata05[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x, y, colorxy);
    showbitmap(bitdata05[(int)((String(numStr.charAt(1)).toInt() + 1) - 1)],xlength,ylength, x+4, y, colorxy);
    showbitmap(bitdata05[(int)((String(numStr.charAt(2)).toInt() + 1) - 1)],xlength,ylength, x+8, y, colorxy);
    showbitmap(bitdata05[(int)((String(numStr.charAt(3)).toInt() + 1) - 1)],xlength,ylength, x+12, y, colorxy);
  }
     }
...
void show15bitnumber(int number, int xlength, int ylength, int x, int y, uint16_t colorxy){
  ...
  if(number<10){
    ...
    showbitmap(bitdata15[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x+7, y, colorxy);
  }else if(number<100){
    showbitmap(bitdata15[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x, y, colorxy);
    showbitmap(bitdata15[(int)((String(numStr.charAt(1)).toInt() + 1) - 1)],xlength,ylength, x+7, y, colorxy);
  }
}
...
void ShowColorNumber(int number, int xlength, int ylength, int x, int y){
  String numStr = String(number);
  if(number<10){
    ShowColorMap(bitdata15[(int)(0)],xlength,ylength, x, y);
    ShowColorMap(bitdata15[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x+7, y);
    }
  else if(number<100){
    ShowColorMap(bitdata15[(int)((String(numStr.charAt(0)).toInt() + 1) - 1)],xlength,ylength, x, y);
    ShowColorMap(bitdata15[(int)((String(numStr.charAt(1)).toInt() + 1) - 1)],xlength,ylength, x+7, y);
     }
}
...
void weatherNow(String url){
 ...
 //   Serial.printf("HTTPGET is %d",httpGet);    //打印服务器状态码
  ...
  //    Serial.println(json);    //打印响应体信息
      ...
      String data1 = doc["location"];
      String data2 = doc["locationDC"];
...
//設定WIFI為AP模式
  ...
  display.clearDisplay();
  scroll_text(4,50,"AP MODE",255,0,0);
  delay(3000); 
  scroll_text(3,50,"192.168.4.1 /index.html",255,0,0);
  delay(4000);  
 }
