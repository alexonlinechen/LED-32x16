...
void showWeather(bool implement){
    ...
    M = tm.tm_min;
            ...
            //天氣圖
        if(tq_code == "0"){       SetEmojiXY(4, 3, 0, 2);   //晴
        }else if(tq_code == "1"){ SetEmojiXY(4, 6, 0, 2);  //多雲
        }else if(tq_code == "2"){ SetEmojiXY(4, 7, 0, 2);  //陰
        }else if(tq_code == "3"){ rain();    //雨
        }else if(tq_code == "4"){ thunderstorm();   //雷雨
        }else if(tq_code == "5"){ windy();       //風           
        }else{ SetEmojiXY(5, 3, 0, 2); }
    
    

    //天氣模式輪播   15秒
    if(millis() - loopTime > 55000 && !implement){
      if(++weatherMode>2) weatherMode=1;
      loopTime = millis();
 ...
 switch(weatherMode){
      ...
      case 2:     
       ...
       // GlobalX = 0;
        //GlobalY = 0;
        show05bitnumber(temp_read.toInt(),3,5,17,2,display.color565(255,48,48));

        cdPattern(23,0,showColor);   // .C 符號
       ...
       // 濕度
        ...
        GlobalY = 7;
        HumPattern();
        show05bitnumber(hum_read.toInt(),3,5,17,10,display.color565(65,105,225));
        GlobalX = 0;
        GlobalY = 0;
        pcPattern(24,8,showColor);  // %符號
        break;
      }
     }
...
void TempPattern(){
  ...
  drawFastYLine(2+GlobalX,2+GlobalY,3,display.color565(255,255,255));
  drawFastYLine(3+GlobalX,2+GlobalY,1,display.color565(255,255,255));
  drawFastYLine(4+GlobalX,2+GlobalY,3,display.color565(255,255,255));
  drawFastYLine(2+GlobalX,5+GlobalY,2,display.color565(255,48,48));
  drawFastYLine(3+GlobalX,4+GlobalY,3,display.color565(255,48,48));
...
void HumPattern(){
  ...
  drawFastYLine(3+GlobalX,4+GlobalY,3,display.color565(65,105,255));
